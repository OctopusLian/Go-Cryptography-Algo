...
func RsaEncryptString(origData string) (string, error) {
	...
	pub := pubInterface.(*rsa.PublicKey)
	ciphertextArr, err := rsa.EncryptPKCS1v15(rand.Reader, pub, []byte(origData))
	if err != nil {
		return "", err
	} else {
		return base64.StdEncoding.EncodeToString(ciphertextArr), nil
	}
}

//解密经过base64处理的加密字符串,返回加密钱的明文
